...
class Generator {
  ...
  async generate(options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      for (let i = 0; i < this.results; i++) {
        ...
        this.include('source', this.randomSource( 
          { 
            mobile: parseInt(options.source_mobile, 10), 
            tv: parseInt(options.source_tv, 10), 
            online: parseInt(options.source_online, 10)
          }) 
        );

        this.include('isvip', this.randomVipStatus( parseInt(options.percentage_vip, 10) ) );
      ...
      if (this.format === 'yaml') {
        resolve({
          output: YAML.stringify(json, 4),
          ext: "yaml"
        });
      } else if (this.format === 'xml') {
        resolve({
          output: js2xmlparser('user', json),
          ext: "xml"
        });
  ...
  randomSource( { mobile, tv, online } = { mobile: 60, tv: 30, online: 10 } ) {
    ...
    let total = mobile + tv + online;

    let ratio_tv = tv / total;
  ...
  randomVipStatus( percentage_vip = 50 ) {
    return Math.random() * 100 > percentage_vip;
  }
  ...
  checkOptions(options) {
    ...
    for (let i = 0; i < keys.length; i++) {
      if (Array.isArray(options[keys[i]])) {
        options[keys[i]] = options[keys[i]].join(",")
      }
    }
  }
...
function numbersInRange(start, end) {
  // console.log( 'doing num in range...' );
  let numbers = [];
